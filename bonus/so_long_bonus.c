...
/*   Updated: 2025/01/02 12:58:41 by aakhrif          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_resolution(t_textures all, t_m_config *map_config, t_window *var)
{
	if (all.wall.img_width * map_config->x_size > MAX_WIDTH)
	{
		...
		free_int_arr(map_config->e_p, var->map_cnfg->enemies_count);
		exit(1);
	}
...
int	main(int ac, char **av)
{
	...
	free_map(map);
	free_int_arr(map_config.e_p, map_config.enemie);
	return (0);
}
